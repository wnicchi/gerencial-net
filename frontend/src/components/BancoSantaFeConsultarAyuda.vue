<!-- BancoSantaFeConsultarAyuda.vue — Ayuda del módulo Bco. Santa Fe - Consultar. -->
<template>
  <div class="bca-ov" @click.self="$emit('close')">
    <div class="bca-modal">
      <button class="bca-x" @click="$emit('close')">✕</button>
      <div class="bca-hero">
        <BancoSantaFeLogo :alto="46" class="bca-logo" />
        <h2>Bco. Santa Fe - Consultar</h2>
...
const pasos = [
  ...
  { t: 'Abrí un lote', d: 'Hacé clic en un lote para ver sus empleados (sucursal, cuenta y monto a acreditar).' },
  { t: 'Generar el movimiento de pagos', d: 'Tildá el/los lote(s), elegí el concepto y el Nº de autorización bancario, y enviá el pago al sistema de Gestión para que GERENCIA lo autorice. No se duplica si ya fue enviado.' },
  { t: 'Ajustar y regenerar', d: 'Sobre los empleados de un lote podés quitar empleados, quitar los de importe cero o editar montos, y “Generar nuevo TXT” para crear un lote nuevo con el archivo del banco.' },
...
.bca-hero { background:linear-gradient(135deg,#1b4332,#40916c); color:#fff; padding:24px 24px 20px; text-align:center; border-radius:14px 14px 0 0; }
.bca-logo { background:#fff; padding:8px 16px; border-radius:10px; }
.bca-hero h2 { margin:12px 0 4px; font-size:18px; } .bca-hero p { margin:0; font-size:13px; opacity:.9; }
...
.bca-num { flex-shrink:0; width:26px; height:26px; border-radius:50%; background:#1b4332; color:#fff; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:700; }
