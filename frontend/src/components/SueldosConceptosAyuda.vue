<!-- SueldosConceptosAyuda.vue — Ayuda del módulo Conceptos de Sueldos. -->
<template>
  <div class="vca-ov" @click.self="$emit('close')">
    <div class="vca-modal">
      <button class="vca-x" @click="$emit('close')">✕</button>
      <div class="vca-hero">
        <svg viewBox="0 0 120 80" class="vca-svg"><rect x="36" y="16" width="48" height="50" rx="4" class="vca-c" /><line x1="44" y1="28" x2="76" y2="28" class="vca-l" /><line x1="44" y1="38" x2="76" y2="38" class="vca-l" /><line x1="44" y1="48" x2="64" y2="48" class="vca-l" /></svg>
        <h2>Conceptos de Sueldos</h2><p>Ítems de las liquidaciones</p>
      </div>
...
defineEmits(['close'])
const pasos = [
  { t: 'Qué define', d: 'El listado de conceptos de liquidación (por ejemplo "Horas Extras 50%", "Antigüedad") que son los ítems de los recibos de sueldo.' },
  { t: 'Crear / editar', d: 'Tocá “Nuevo concepto” o uno de la lista, escribí el nombre y guardá.' },
  { t: 'Eliminar', d: 'Con el concepto seleccionado, usá “Eliminar”.' },
]
</script>
<style scoped>
...
.vca-modal { background:#fff; width:500px; max-width:94vw; max-height:90vh; overflow:auto; border-radius:14px; position:relative; box-shadow:0 20px 60px rgba(0,0,0,.3); animation:vca-pop .25s cubic-bezier(.2,.9,.3,1.2); }
