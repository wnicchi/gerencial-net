<!-- DepartamentosView.vue — ABM de Departamentos (tabla departam). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">🏢</div>
      <div class="cs-tx"><h1>Departamentos</h1><p>{{ lista.length }} departamento{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/departamentos" titulo="Asistente IA — Departamentos"
            subtitulo="Preguntá sobre el ABM de departamentos"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego un departamento?','¿Por qué no me deja eliminar uno?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin departamentos cargados</div>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar departamento' : 'Nuevo departamento' }}</h3>
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Departamentos</h3>
          <ul class="cs-help">
            <li>Administra los <b>departamentos</b> que después se asignan a los puestos de trabajo.</li>
            <li>El <b>código</b> lo asigna el sistema; el <b>nombre</b> es obligatorio.</li>
            <li>No se puede eliminar un departamento con puestos asignados.</li>
          </ul>
...
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/departamentos')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) { await api.put(`/departamentos/${form.value.cod}`, { nombre: form.value.nombre }); flash('Departamento actualizado') }
    else { await api.post('/departamentos', { nombre: form.value.nombre }); flash('Departamento creado') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar el departamento "${e.nombre}"?`)) return
  try { await api.delete(`/departamentos/${e.cod}`); flash('Departamento eliminado'); await cargar() }
