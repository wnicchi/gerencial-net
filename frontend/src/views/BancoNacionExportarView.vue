<!-- BancoNacionExportarView.vue — Bco. Nación - Exportar (empleados_exportar_bco_nacion). -->
<template>
  <div class="bs-view">
    <div class="bs-cab">
      <div class="bs-cab-ico">📤</div>
      <div class="bs-cab-tx"><h1>Bco. Nación - Exportar</h1><p>Archivo de acreditación de sueldos para el Banco de la Nación Argentina (por CBU)</p></div>
      <BancoNacionLogo :alto="34" class="bs-logo" />
    ...
    <BancoNacionExportarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-nacion" titulo="Asistente IA — Bco. Nación Exportar"
    ...
    <div v-if="cargado && filas.length" class="bs-pie">
      <div class="bs-gen">
        ...
        <button class="bs-btn-gen" :disabled="generando" @click="generar">{{ generando ? '⟳…' : 'GENERAR TXT' }}</button>
      </div>
...
import BancoNacionExportarAyuda from '@/components/BancoNacionExportarAyuda.vue'
...
const f = reactive({ empresa: 0, contratista: 0, lugar: 0, convenio: 0, categoria: 0, empleado: 0, mes: _prev.getMonth() + 1, anio: _prev.getFullYear(), tipoRemu: 0, tipoSueldo: 0, banco: 3, anticipos: false })
...
const consultar = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/bco-nacion/consultar', { params })
...
const generar = async () => {
  ...
  try {
    ...
    const { data } = await api.post('/bco-nacion/generar', payload)
...
onMounted(async () => {
  ...
  const nac = cat.bancos.find((b: any) => b.nombre.toUpperCase().includes('NACION'))
  if (nac) f.banco = nac.cod
})
</script>
...
.bs-f { display:flex; flex-direction:column; gap:4px; position:relative; } .bs-lbl { font-size:12px; font-weight:700; color:#155560; }
...
.bs-result li { padding:7px 10px; font-size:13px; cursor:pointer; color:#1e293b; border-bottom:1px solid #f1f5f9; } .bs-result li:hover { background:#ecfeff; }
.bs-chk { display:flex; align-items:center; gap:6px; font-size:13px; color:#1e293b; }
.bs-btn-ok { background:#155560; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-ok:disabled { background:#cbd5e1; }
...
.bs-grid th { position:sticky; top:0; background:#155560; color:#fff; font-weight:600; padding:7px 10px; font-size:12px; text-align:left; } .bs-grid th.r { text-align:right; }
...
.bs-tfoot td { background:#0e3d45 !important; color:#fff !important; font-weight:800; padding:9px 10px; }
...
.bs-total-box { margin-left:auto; display:flex; align-items:center; gap:12px; background:linear-gradient(120deg,#0e3d45,#1d6a78); color:#fff; padding:9px 16px; border-radius:9px; box-shadow:0 6px 18px rgba(14,61,69,.35); }
...
.bs-btn-gen { background:#1d6a78; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-gen:disabled { background:#cbd5e1; }
