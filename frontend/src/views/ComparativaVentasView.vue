<template>
  <div class="cv">
    ...
    <div class="cv-filtros">
      <div class="cv-f">
        <span class="cv-lbl">Rubro</span>
        <label class="cv-radio"><input type="radio" :value="0" v-model.number="modoRubro" /> Todos</label>
        <label class="cv-radio"><input type="radio" :value="1" v-model.number="modoRubro" /> Uno</label>
        <select v-if="modoRubro === 1" v-model.number="rubro" class="cv-sel">
          <option :value="0">— Elegí —</option>
          <option v-for="r in rubros" :key="r.cod" :value="r.cod">{{ r.des }}</option>
        </select>
      </div>
      <div class="cv-f">
        <span class="cv-lbl">Moneda</span>
        <label class="cv-radio"><input type="radio" value="P" v-model="moneda" /> Pesos</label>
        <label class="cv-radio"><input type="radio" value="D" v-model="moneda" /> Dólares</label>
      </div>
      <div class="cv-f">
        <span class="cv-lbl">Desde</span>
        <select v-model.number="desdeMes" class="cv-sel mes">
          <option v-for="(m, i) in MESES" :key="i" :value="i + 1">{{ m }}</option>
        </select>
        <input v-model.number="desdeAnio" type="number" class="cv-anio" :min="anioMin" :max="anioHoy" />
      </div>
      <label class="cv-chk"><input type="checkbox" v-model="incluyeIva" /> Incluye I.V.A.</label>
    ...
    <template v-if="pres.length">
      <div class="cv-tit">Ventas en {{ monedaLbl }}{{ incluyeIva ? ' (con IVA)' : '' }}</div>
      <div class="cv-scroll">
    ...
    <Teleport to="body">
      <div v-if="verGrafico" class="cv-ov" @click.self="verGrafico = false">
        <div class="cv-modal">
          <div class="cv-mhead"><span>Gráfica de ventas ({{ monedaLbl }})</span><button class="cv-x" @click="verGrafico = false">✕</button></div>
...
/**
 * ComparativaVentasView.vue — Estadísticas › Comparativa de Ventas.
 * Informe (solo lectura): tabla de ventas por mes/año + variación interanual %,
 * export a Excel y gráfico SVG. Migra estadistica_comparativa_ventas.scx.
 */
...
const anioMin = ref(2007)
const anioHoy = ref(new Date().getFullYear())

const modoRubro = ref(0)
const rubro = ref(0)
const moneda = ref<'P' | 'D'>('P')
const desdeMes = ref(1)
const desdeAnio = ref(new Date().getFullYear() - 2)
const incluyeIva = ref(false)
...
const monedaLbl = computed(() => (moneda.value === 'D' ? 'DÓLARES' : 'PESOS'))

function aviso (t: string, err = false) { msg.value = t; msgErr.value = err; clearTimeout(msgT); msgT = setTimeout(() => (msg.value = ''), 5000) }
...
async function cargarCatalogo () {
  try {
    const d = (await api.get('/tablero/gestion/comparativa-ventas/catalogo')).data
    rubros.value = d.rubros; anioMin.value = d.anio_min; anioHoy.value = d.anio_hoy
    if (desdeAnio.value < anioMin.value) desdeAnio.value = anioMin.value
  } catch { /* noop */ }
}

async function generar () {
  if (modoRubro.value === 1 && rubro.value <= 0) { aviso('Elegí un rubro.', true); return }
  cargando.value = true
  try {
    const params = {
      rubro: modoRubro.value === 1 ? rubro.value : 0,
      moneda: moneda.value,
      desde_mes: desdeMes.value,
      desde_anio: desdeAnio.value,
      incluye_iva: incluyeIva.value ? 1 : 0,
    }
    const d = (await api.get('/tablero/gestion/comparativa-ventas', { params })).data
    pres.value = d.presentacion; inter.value = d.interanual
    if (!pres.value.length) aviso('Sin datos para el período elegido.')
  } catch (err: any) {
    pres.value = []; inter.value = []
    const r = err?.response?.data
    aviso(r?.error ? r.error + (r.detalles?.length ? ' (' + r.detalles.length + ' comprobantes)' : '') : 'No se pudo generar el informe.', true)
  } finally { cargando.value = false }
}

function aExcel () {
  const filas: any[] = []
  const enc: any = { 'AÑO': 'Ventas ' + monedaLbl.value }
  filas.push({ AÑO: '', ...Object.fromEntries(MESES.map(m => [m, m])), 'Total Anual': 'Total Anual' })
  ...
  guardarDesdeUrl(url, 'estadistica_ventas.xlsx')
  setTimeout(() => URL.revokeObjectURL(url), 4000)
}

// ── Gráfico SVG (multi-línea: una línea por año, 12 meses) ──
const COLORES = ['#1b4332', '#2d6a9f', '#d97706', '#7c3aed', '#dc2626', '#0891b2', '#65a30d']
const svgGrafico = computed(() => {
  if (!pres.value.length) return ''
  const W = 820, H = 420, mL = 70, mR = 20, mT = 20, mB = 60
  ...
  for (let t = 0; t <= 4; t++) {
    ...
    g += `<line x1="${mL}" y1="${yy}" x2="${W - mR}" y2="${yy}" stroke="#e2e8f0"/>`
    g += `<text x="${mL - 8}" y="${yy + 4}" text-anchor="end" font-size="11" fill="#64748b">${fmtM(v)}</text>`
  }
  ...
  pres.value.forEach((p, idx) => {
    ...
    const pts = p.meses.map((v, i) => `${x(i)},${y(v)}`).join(' ')
    g += `<polyline points="${pts}" fill="none" stroke="${col}" stroke-width="2.5"/>`
    p.meses.forEach((v, i) => { g += `<circle cx="${x(i)}" cy="${y(v)}" r="3" fill="${col}"/>` })
  })
  // leyenda
  pres.value.forEach((p, idx) => {
    const col = COLORES[idx % COLORES.length], lx = mL + idx * 90, ly = mT + 4
    g += `<rect x="${lx}" y="${ly - 9}" width="12" height="12" fill="${col}"/><text x="${lx + 16}" y="${ly + 1}" font-size="12" fill="#334155">${p.anio}</text>`
  })
  g += '</svg>'
  return g
})
...
.cv-radio, .cv-chk { display: inline-flex; align-items: center; gap: 5px; font-size: 13px; color: #1e293b; cursor: pointer; }
.cv-sel, .cv-anio { border: 1px solid #cbd5e1; border-radius: 7px; padding: 6px 9px; font-size: 13px; color: #1e293b; background: #fff; }
.cv-sel.mes { width: 110px; } .cv-anio { width: 80px; }
