<!-- BancoNacionConsultarView.vue — Bco. Nación - Consultar (empleados_consultar_bco_nacion). -->
<template>
  <div class="bc-view">
    <div class="bc-cab">
      <div class="bc-cab-ico">🔍</div>
      <div class="bc-cab-tx"><h1>Bco. Nación - Consultar</h1><p>Lotes de pago exportados — autorización y regeneración</p></div>
      <BancoNacionLogo :alto="32" class="bc-logo" />
    ...
    <BancoNacionConsultarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-nacion-consultar" titulo="Asistente IA — Bco. Nación Consultar"
    ...
    <div class="bc-cols">
      <div class="bc-panel">
        <div class="bc-panel-tit">Lotes (Haberes Bco. Nación)</div>
        <div class="bc-wrap">
          <table class="bc-grid">
    ...
    <div class="bc-acciones">
      ...
      <div class="bc-derecha">
        ...
        <div class="bc-gen">
          ...
          <button class="bc-btn-txt" :disabled="generando || !loteActivo" @click="generarNuevo">{{ generando ? '⟳…' : 'GENERAR NUEVAMENTE' }}</button>
        </div>
      </div>
    </div>
...
import BancoNacionConsultarAyuda from '@/components/BancoNacionConsultarAyuda.vue'
...
const cargarLotes = async () => {
  cargando.value = true
  try { lotes.value = (await api.get('/bco-nacion/lotes', { params: { empresa: empresa.value } })).data.lotes || []; bene.value = []; loteActivo.value = 0 }
...
const abrirLote = async (l: Lote) => {
  ...
  try { bene.value = (await api.get('/bco-nacion/beneficiarios', { params: { lote: l.nro } })).data.beneficiarios || [] }
...
const generarMovimiento = async () => {
  ...
  try {
    const { data } = await api.post('/bco-nacion/generar-movimiento', {
...
const eliminarLotes = async () => {
  ...
  if (!confirm(`¿Anular el lote ${sel[0].nro} de bchaberes?`)) return
  try {
    await api.post('/bco-nacion/eliminar', { lotes: sel.map(l => l.nro), nroBancario: movimiento.nroBancario || 0 })
...
const generarNuevo = async () => {
  ...
  try {
    const { data } = await api.post('/bco-nacion/generar-nuevo', {
...
.bc-f { display:flex; flex-direction:column; gap:4px; } .bc-lbl { font-size:12px; font-weight:700; color:#155560; }
.bc-sel { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; min-width:200px; color:#1e293b; }
.bc-btn-ok { background:#155560; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-ok:disabled { background:#cbd5e1; }
...
.bc-panel-tit { background:#155560; color:#fff; font-size:12px; font-weight:700; padding:7px 12px; }
...
.bc-grid tbody tr.activo td { outline:2px solid #1d6a78; outline-offset:-2px; }
...
.bc-tfoot td { background:#0e3d45 !important; color:#fff !important; font-weight:800; padding:7px 8px; }
...
.bc-btn-mov { background:#155560; color:#fff; border:none; padding:12px; border-radius:8px; cursor:pointer; font-size:13px; font-weight:800; } .bc-btn-mov:disabled { background:#cbd5e1; }
...
.bc-btn-txt { margin-left:auto; background:#1d6a78; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-txt:disabled { background:#cbd5e1; }
