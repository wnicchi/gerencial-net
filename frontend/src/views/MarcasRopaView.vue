<!-- MarcasRopaView.vue — ABM de Marcas de Ropa/EPP (tabla ropamarca). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">🏷️</div>
      <div class="cs-tx"><h1>Marcas de Ropa/EPP</h1><p>{{ lista.length }} marca{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/marcas-ropa" titulo="Asistente IA — Marcas de Ropa/EPP"
            subtitulo="Preguntá sobre el ABM de marcas"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego una marca?','¿Dónde se usan?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin marcas cargadas</div>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar marca' : 'Nueva marca' }}</h3>
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Marcas de Ropa/EPP</h3>
          <ul class="cs-help">
            <li>Administra las <b>marcas</b> de las prendas y elementos de protección (ej. Dupont, Grafa).</li>
...
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/marcas-ropa')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) { await api.put(`/marcas-ropa/${form.value.cod}`, { nombre: form.value.nombre }); flash('Marca actualizada') }
    else { await api.post('/marcas-ropa', { nombre: form.value.nombre }); flash('Marca creada') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar la marca "${e.nombre}"?`)) return
  try { await api.delete(`/marcas-ropa/${e.cod}`); flash('Marca eliminada'); await cargar() }
