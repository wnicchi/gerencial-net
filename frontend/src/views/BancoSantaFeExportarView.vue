<!-- BancoSantaFeExportarView.vue — Bco. Santa Fe - Exportar (empleados_exportar_bco_sf). -->
<template>
  <div class="bs-view">
    <div class="bs-cab">
      <div class="bs-cab-ico">📤</div>
      <div class="bs-cab-tx"><h1>Bco. Santa Fe - Exportar</h1><p>Archivo de acreditación de sueldos para el Nuevo Banco de Santa Fe</p></div>
      <BancoSantaFeLogo :alto="40" class="bs-logo" />
    ...
    <BancoSantaFeAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-santa-fe" titulo="Asistente IA — Bco. Santa Fe Exportar"
            subtitulo="Preguntá sobre la exportación al banco"
            :sugerencias="['¿Por qué un empleado figura en cero?','¿Qué significan las filas rojas?','¿Qué es la fecha de imputación?']"
            @close="modalIA = false" />
    ...
    <div v-if="cargado && filas.length" class="bs-acciones">
      ...
      <span v-if="sinCuentaCount" class="bs-warn">⚠ {{ sinCuentaCount }} sin sucursal/cuenta</span>
    </div>
    ...
    <div v-if="cargado && filas.length" class="bs-wrap">
      <table class="bs-grid">
        <thead><tr><th><input type="checkbox" :checked="todosMarcados" @change="todos(($event.target as HTMLInputElement).checked)" /></th>
          <th>Código</th><th>Nombre</th><th>T.Doc</th><th>Nro.Doc</th><th>Cuil</th><th class="r">Suc.B.S.F.</th><th class="r">Cuenta B.S.F.</th><th class="r">Monto a Depositar</th></tr></thead>
        <tbody>
          <tr v-for="(r, i) in filas" :key="i" :class="{ sel: r.elegir, sincta: r.sinCuenta }">
        ...
        <tfoot><tr class="bs-tfoot"><td colspan="8">TOTAL A DEPOSITAR</td><td class="r">{{ money(total) }}</td></tr></tfoot>
      </table>
    </div>
    ...
    <div v-if="cargado && filas.length" class="bs-pie">
      <div class="bs-gen">
        ...
        <button class="bs-btn-gen" :disabled="generando" @click="generar">{{ generando ? '⟳ Generando…' : 'GENERAR TXT PARA EL BANCO' }}</button>
      </div>
...
import BancoSantaFeAyuda from '@/components/BancoSantaFeAyuda.vue'
import BancoSantaFeLogo from '@/components/BancoSantaFeLogo.vue'
import ChatIA from '@/components/ChatIA.vue'
import { guardarComo } from '@/utils/descargas'

interface Fila { elegir: boolean; cod: number; nombre: string; tdoc: string; ndoc: string; cuil: string; suc: number; cta: number; monto: number; anti: number; sinCuenta: boolean }
...
const f = reactive({ empresa: 0, contratista: 0, lugar: 0, convenio: 0, categoria: 0, empleado: 0, mes: _prev.getMonth() + 1, anio: _prev.getFullYear(), tipoRemu: 0, tipoSueldo: 0, banco: 2, anticipos: false })
const gen = reactive({ fechaEnvio: _iso(hoy), fechaImputacion: '', modalidad: '0005' })
...
const recalc = () => { total.value = filas.value.reduce((a, r) => a + r.monto, 0) }
const sinCuentaCount = computed(() => filas.value.filter(r => r.sinCuenta).length)
...
const consultar = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/bco-santa-fe/consultar', { params })
...
const generar = async () => {
  ...
  if (!gen.modalidad.trim()) return flash('Indicá la modalidad.', true)
  const sinCta = filas.value.filter(r => r.sinCuenta).length
  if (sinCta && !confirm(`Hay ${sinCta} empleado(s) sin sucursal/cuenta del banco. Se exportarán con cuenta 0. ¿Continuar igual?`)) return
  if (!confirm('Se generará el TXT y se registrará el lote. ¿Continuar?')) return
  generando.value = true
  try {
    const payload = {
      empresa: f.empresa, mes: f.mes, anio: f.anio,
      fechaEnvio: gen.fechaEnvio, fechaImputacion: gen.fechaImputacion, modalidad: gen.modalidad.trim(),
      liquidacionModo: f.tipoSueldo ? 2 : 1, tipoSueldo: f.tipoSueldo,
      filas: filas.value.map(r => ({ cod: r.cod, nombre: r.nombre, suc: r.suc, cta: r.cta, monto: r.monto })),
    }
    const { data } = await api.post('/bco-santa-fe/generar', payload)
    await guardarComo(new Blob([data.contenido], { type: 'text/plain' }), data.filename)
    flash(`Archivo ${data.filename} generado. Lote ${data.lote}, ${data.lineas} líneas, total ${money(data.total)}.`)
...
onMounted(async () => {
  ...
  const sf = cat.bancos.find((b: any) => b.nombre.toUpperCase().includes('SANTA FE'))
  if (sf) f.banco = sf.cod
})
</script>
...
.bs-logo { margin-left:18px; padding:5px 12px; background:#fff; border:1px solid #e2e8f0; border-radius:8px; }
...
.bs-f { display:flex; flex-direction:column; gap:4px; position:relative; } .bs-lbl { font-size:12px; font-weight:700; color:#1b4332; }
...
.bs-result li { padding:7px 10px; font-size:13px; cursor:pointer; color:#1e293b; border-bottom:1px solid #f1f5f9; } .bs-result li:hover { background:#f0faf4; }
.bs-chk { display:flex; align-items:center; gap:6px; font-size:13px; color:#1e293b; }
.bs-btn-ok { background:#16a34a; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-ok:disabled { background:#cbd5e1; }
...
.bs-grid th { position:sticky; top:0; background:#1b4332; color:#fff; font-weight:600; padding:7px 10px; font-size:12px; text-align:left; } .bs-grid th.r { text-align:right; }
...
.bs-tfoot td { background:#14532d !important; color:#fff !important; font-weight:800; padding:9px 10px; }
...
.bs-total-box { margin-left:auto; display:flex; align-items:center; gap:12px; background:linear-gradient(120deg,#14532d,#16a34a); color:#fff; padding:9px 16px; border-radius:9px; box-shadow:0 6px 18px rgba(20,83,45,.35); }
...
.bs-mod { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; width:90px; color:#1e293b; }
.bs-btn-gen { background:#2563eb; color:#fff; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-gen:disabled { background:#cbd5e1; }
