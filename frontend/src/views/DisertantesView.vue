<!-- DisertantesView.vue — ABM de Disertantes (tabla disertantes). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">🎤</div>
      <div class="cs-tx"><h1>Disertantes</h1><p>{{ lista.length }} disertante{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/disertantes" titulo="Asistente IA — Disertantes"
            subtitulo="Preguntá sobre el ABM de disertantes"
            :sugerencias="['¿Para qué sirve este módulo?','¿Qué datos cargo de un disertante?','¿Dónde se usan?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card big">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre o email…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin disertantes cargados</div>
      <table v-else class="cs-tabla">
        <thead><tr>
          ...
          <th class="ord" @click="ordenar('email')">Email{{ flecha('email') }}</th>
          <th style="width:100px;text-align:center">Acciones</th>
        </tr></thead>
        <tbody>
          <tr v-for="(e, i) in listaFiltrada" :key="i">
            <td class="cs-cod">{{ e.cod }}</td><td>{{ e.nombre }}</td><td>{{ e.domicilio }}</td><td>{{ e.telefono }}</td><td>{{ e.email }}</td>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar disertante' : 'Nuevo disertante' }}</h3>
          <label>Nombre *</label>
          <input ref="inputNom" v-model="form.nombre" maxlength="50" />
          <label>Domicilio</label>
          <input v-model="form.domicilio" maxlength="50" />
          <label>Teléfonos</label>
          <input v-model="form.telefono" maxlength="50" />
          <label>Email</label>
          <input v-model="form.email" maxlength="50" @keyup.enter="guardar" />
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Disertantes</h3>
          <ul class="cs-help">
            <li>Administra los <b>disertantes</b> (quienes dictan las capacitaciones).</li>
            <li>De cada uno se carga nombre (obligatorio), domicilio, teléfonos y email.</li>
            <li>El <b>código</b> lo asigna el sistema. Se eligen al cargar o dar resultado a una jornada.</li>
          </ul>
...
interface Item { cod: number; nombre: string; domicilio: string; telefono: string; email: string }
...
const vacio = (): any => ({ cod: null, nombre: '', domicilio: '', telefono: '', email: '' })
...
const listaFiltrada = computed(() => {
  ...
  const base = q ? lista.value.filter(e => `${e.nombre} ${e.email}`.toLowerCase().includes(q)) : lista.value.slice()
...
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/disertantes')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  const body = { nombre: form.value.nombre, domicilio: form.value.domicilio, telefono: form.value.telefono, email: form.value.email }
  try {
    if (editando.value) { await api.put(`/disertantes/${form.value.cod}`, body); flash('Disertante actualizado') }
    else { await api.post('/disertantes', body); flash('Disertante creado') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar al disertante "${e.nombre}"?`)) return
  try { await api.delete(`/disertantes/${e.cod}`); flash('Disertante eliminado'); await cargar() }
...
function imprimir () {
  ...
  doc.text('DISERTANTES', PW / 2, y, { align: 'center' }); doc.setTextColor(0, 0, 0); y += 10
  ...
  cerrarPdf(); pdfNombre.value = 'Lista_disertantes.pdf'; pdfUrl.value = URL.createObjectURL(doc.output('blob'))
}
onMounted(cargar)
</script>
