<!-- BancoSantanderRioConsultarView.vue — Bco. Santander Río - Consultar (empleados_consultar_bco_santander_rio). -->
<template>
  <div class="bc-view">
    <div class="bc-cab">
      <div class="bc-cab-ico">🔍</div>
      <div class="bc-cab-tx"><h1>Bco. Santander Río - Consultar</h1><p>Lotes de pago exportados — autorización y regeneración</p></div>
      <BancoSantanderLogo :alto="36" class="bc-logo" />
    ...
    <BancoSantanderRioConsultarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-santander-rio-consultar" titulo="Asistente IA — Bco. Santander Río Consultar"
    ...
    <div class="bc-cols">
      <div class="bc-panel">
        <div class="bc-panel-tit">Lotes (Haberes Bco. Santander Río)</div>
        <div class="bc-wrap">
          <table class="bc-grid">
...
import BancoSantanderRioConsultarAyuda from '@/components/BancoSantanderRioConsultarAyuda.vue'
...
// Un solo lote a la vez; el Nº de autorización = número del lote.
...
const cargarLotes = async () => {
  cargando.value = true
  try { lotes.value = (await api.get('/bco-santander-rio/lotes', { params: { empresa: empresa.value } })).data.lotes || []; bene.value = []; loteActivo.value = 0 }
...
const abrirLote = async (l: Lote) => {
  ...
  try { bene.value = (await api.get('/bco-santander-rio/beneficiarios', { params: { lote: l.nro } })).data.beneficiarios || [] }
...
const generarMovimiento = async () => {
  ...
  try {
    const { data } = await api.post('/bco-santander-rio/generar-movimiento', {
...
const eliminarLotes = async () => {
  ...
  if (!confirm(`¿Anular el lote ${sel[0].nro} de SRHABERES?`)) return
  try {
    await api.post('/bco-santander-rio/eliminar', { lotes: sel.map(l => l.nro), nroBancario: movimiento.nroBancario || 0 })
...
const generarNuevo = async (tipo: 'txt' | 'excel') => {
  ...
  try {
    const { data } = await api.post('/bco-santander-rio/generar-nuevo', {
...
.bc-btn-txt { background:#2563eb; color:#fff; border:none; padding:10px 16px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-txt.excel { background:#16a34a; } .bc-btn-txt:disabled { background:#cbd5e1; }
