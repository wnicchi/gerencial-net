<!-- BancoFrancesConsultarView.vue — Bco. Francés (BBVA) - Consultar (empleados_consultar_bco_frances). -->
<template>
  <div class="bc-view">
    <div class="bc-cab">
      <div class="bc-cab-ico">🔍</div>
      <div class="bc-cab-tx"><h1>Bco. Francés (BBVA) - Consultar</h1><p>Lotes de pago exportados — autorización y regeneración</p></div>
      <BancoFrancesLogo :alto="32" class="bc-logo" />
    ...
    <BancoFrancesConsultarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-frances-consultar" titulo="Asistente IA — Bco. Francés Consultar"
    ...
    <div class="bc-cols">
      <div class="bc-panel">
        <div class="bc-panel-tit">Lotes (Haberes Bco. Francés)</div>
        <div class="bc-wrap">
          <table class="bc-grid">
    ...
    <div class="bc-acciones">
      ...
      <div class="bc-derecha">
        ...
        <div class="bc-gen">
          ...
          <button class="bc-btn-txt" :disabled="generando || !loteActivo" @click="generarNuevo">{{ generando ? '⟳…' : 'GENERAR NUEVO TXT' }}</button>
        </div>
      </div>
    </div>
...
import { svgMarkupToPng } from '@/utils/svgToPng'
import BancoFrancesConsultarAyuda from '@/components/BancoFrancesConsultarAyuda.vue'
...
const cargarLotes = async () => {
  cargando.value = true
  try { lotes.value = (await api.get('/bco-frances/lotes', { params: { empresa: empresa.value } })).data.lotes || []; bene.value = []; loteActivo.value = 0 }
...
const abrirLote = async (l: Lote) => {
  ...
  try { bene.value = (await api.get('/bco-frances/beneficiarios', { params: { lote: l.nro } })).data.beneficiarios || [] }
...
const generarMovimiento = async () => {
  ...
  try {
    const { data } = await api.post('/bco-frances/generar-movimiento', {
...
const eliminarLotes = async () => {
  ...
  if (!confirm(`¿Anular el lote ${sel[0].nro} de FRHABERES?`)) return
  try {
    await api.post('/bco-frances/eliminar', { lotes: sel.map(l => l.nro), nroBancario: movimiento.nroBancario || 0 })
...
const generarNuevo = async () => {
  ...
  try {
    const { data } = await api.post('/bco-frances/generar-nuevo', {
      loteOrigen: loteActivo.value, fechaEnvio: gen.fechaEnvio, fechaImputacion: gen.fechaImputacion,
      beneficiarios: bene.value.map(b => ({ emp: b.emp, mon: b.mon })),
    })
...
const imprimirPdf = async () => {
  ...
  // El logo de BBVA en pantalla es HTML/CSS; armo un SVG equivalente para el PDF.
  let bancoLogoUrl: string | undefined; let bancoLogoRatio = 3.4
  const bbva = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 340 100"><rect x="0" y="18" width="150" height="64" rx="6" fill="#072146"/><text x="75" y="63" font-family="Arial,Helvetica,sans-serif" font-size="40" font-weight="bold" fill="#fff" text-anchor="middle">BBVA</text><text x="162" y="63" font-family="Arial,Helvetica,sans-serif" font-size="30" font-weight="bold" fill="#072146">Banco Francés</text></svg>`
  const png = await svgMarkupToPng(bbva, 340, 100)
  if (png) { bancoLogoUrl = png.url; bancoLogoRatio = png.ratio }
...
.bc-f { display:flex; flex-direction:column; gap:4px; } .bc-lbl { font-size:12px; font-weight:700; color:#072146; }
.bc-sel { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; min-width:200px; color:#1e293b; }
.bc-btn-ok { background:#072146; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-ok:disabled { background:#cbd5e1; }
...
.bc-panel-tit { background:#072146; color:#fff; font-size:12px; font-weight:700; padding:7px 12px; }
...
.bc-grid tbody tr.activo td { outline:2px solid #1e4fa0; outline-offset:-2px; }
...
.bc-tfoot td { background:#0a2e5e !important; color:#fff !important; font-weight:800; padding:7px 8px; }
...
.bc-btn-mov { background:#072146; color:#fff; border:none; padding:12px; border-radius:8px; cursor:pointer; font-size:13px; font-weight:800; } .bc-btn-mov:disabled { background:#cbd5e1; }
...
.bc-btn-txt { margin-left:auto; background:#1e4fa0; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-txt:disabled { background:#cbd5e1; }
