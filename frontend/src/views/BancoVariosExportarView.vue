<!-- BancoVariosExportarView.vue — Bcos. Varios (Santander) - Exportar (empleados_exportar_bco_varios). -->
<template>
  <div class="bs-view">
    <div class="bs-cab">
      <div class="bs-cab-ico">📤</div>
      <div class="bs-cab-tx"><h1>Bcos. Varios (Santander) - Exportar</h1><p>Acreditación por CBU a otros bancos (no incluye Santander Río, Santa Fe ni Francés)</p></div>
      <BancoSantanderLogo :alto="36" class="bs-logo" />
    ...
    <BancoVariosExportarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-varios" titulo="Asistente IA — Bcos. Varios (Santander)"
            subtitulo="Preguntá sobre la exportación a bancos varios"
            :sugerencias="['¿Qué bancos incluye?','¿Qué significan las filas rojas?','¿TXT o Excel?']"
            @close="modalIA = false" />

    <div class="bs-aviso">⚠ No incluye empleados de los bancos: <b>Santander Río, Santa Fe ni Francés</b> (esos tienen su propio módulo). Se paga a los demás por CBU vía Santander.</div>
    ...
    <div v-else-if="cargado" class="bs-vacio">No se encontraron empleados de bancos varios para los filtros seleccionados.</div>
    ...
    <div v-if="cargado && filas.length" class="bs-pie">
      <div class="bs-gen">
        ...
        <div class="bs-f"><span class="bs-lbl">Orden de archivo del día</span><input v-model.number="gen.orden" type="number" class="bs-orden" /></div>
        <label class="bs-chk"><input type="checkbox" v-model="gen.enviaAlBanco" /> Archivo para enviar al banco</label>
        <button class="bs-btn-gen" :disabled="generando" @click="generar('txt')">{{ generando ? '⟳…' : 'GENERAR TXT' }}</button>
        <button class="bs-btn-gen excel" :disabled="generando" @click="generar('excel')">{{ generando ? '⟳…' : 'GENERAR EXCEL' }}</button>
      </div>
...
import BancoVariosExportarAyuda from '@/components/BancoVariosExportarAyuda.vue'
...
interface Fila { elegir: boolean; cod: number; nombre: string; tdoc: string; ndoc: string; cuil: string; suc: number; cta: number; monto: number; anti: number; dom: string; cbu: string; sinCuenta: boolean; sinCbu: boolean }
...
const f = reactive({ empresa: 0, contratista: 0, lugar: 0, convenio: 0, categoria: 0, empleado: 0, mes: _prev.getMonth() + 1, anio: _prev.getFullYear(), tipoRemu: 0, tipoSueldo: 0, anticipos: false })
const gen = reactive({ fechaEnvio: _iso(hoy), fechaImputacion: '', orden: 1, enviaAlBanco: false })
const cat = reactive<{ empresas: any[]; contratistas: any[]; lugares: any[]; convenios: any[]; categorias: any[]; tipos: any[] }>({ empresas: [], contratistas: [], lugares: [], convenios: [], categorias: [], tipos: [] })
...
const consultar = async () => {
  ...
  try {
    const params: any = {
      empresa: f.empresa, mes: f.mes, anio: f.anio, anticipos: f.anticipos ? 1 : 0,
    ...
    const { data } = await api.get('/bco-varios/consultar', { params })
...
const generar = async (tipo: 'txt' | 'excel') => {
  ...
  if (!confirm(`Se generará el ${tipo.toUpperCase()} y se registrará el lote. ¿Continuar?`)) return
  generando.value = true
  try {
    const payload = {
      tipo, empresa: f.empresa, mes: f.mes, anio: f.anio,
      fechaEnvio: gen.fechaEnvio, fechaImputacion: gen.fechaImputacion, modalidad: '', orden: gen.orden, enviaAlBanco: gen.enviaAlBanco,
      liquidacionModo: f.tipoSueldo ? 2 : 1, tipoSueldo: f.tipoSueldo,
      filas: filas.value.map(r => ({ cod: r.cod, nombre: r.nombre, cuil: r.cuil, dom: r.dom, cbu: r.cbu, suc: r.suc, cta: r.cta, monto: r.monto })),
    }
    const { data } = await api.post('/bco-varios/generar', payload)
...
.bs-aviso { margin:12px 18px 0; padding:9px 14px; background:#fff7ed; border:1px solid #fed7aa; border-radius:8px; font-size:12.5px; color:#9a3412; }
.bs-filtros { display:flex; flex-wrap:wrap; align-items:flex-end; gap:12px; padding:14px 18px; }
.bs-f { display:flex; flex-direction:column; gap:4px; position:relative; } .bs-lbl { font-size:12px; font-weight:700; color:#b30000; }
...
.bs-orden { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; width:80px; background:#fef9c3; }
...
.bs-result li { padding:7px 10px; font-size:13px; cursor:pointer; color:#1e293b; border-bottom:1px solid #f1f5f9; } .bs-result li:hover { background:#fff1f1; }
.bs-chk { display:flex; align-items:center; gap:6px; font-size:13px; color:#1e293b; }
.bs-btn-ok { background:#b30000; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-ok:disabled { background:#cbd5e1; }
...
.bs-grid th { position:sticky; top:0; background:#b30000; color:#fff; font-weight:600; padding:7px 10px; font-size:12px; text-align:left; } .bs-grid th.r { text-align:right; }
...
.bs-tfoot td { background:#7f1d1d !important; color:#fff !important; font-weight:800; padding:9px 10px; }
...
.bs-total-box { margin-left:auto; display:flex; align-items:center; gap:12px; background:linear-gradient(120deg,#7f1d1d,#ec0000); color:#fff; padding:9px 16px; border-radius:9px; box-shadow:0 6px 18px rgba(127,29,29,.35); }
...
.bs-btn-gen { background:#b30000; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-gen.excel { background:#16a34a; } .bs-btn-gen:disabled { background:#cbd5e1; }
