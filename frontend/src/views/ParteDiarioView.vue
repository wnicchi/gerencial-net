<!-- ParteDiarioView.vue — Reloj / Parte Diario de Horas (reloj_horarios_parte_diario). -->
<template>
  <div class="ht-view">
    <div class="ht-cab">
      <div class="ht-cab-ico">📋</div>
      <div class="ht-cab-tx"><h1>Parte Diario de Horas</h1><p>Quiénes registraron entrada en una fecha y sus observaciones</p></div>
    ...
    <ParteDiarioAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/parte-diario" titulo="Asistente IA — Parte Diario"
            subtitulo="Preguntá sobre el parte del día" :sugerencias="['¿Qué muestra el parte?','¿Qué son los no registrados?','¿De dónde salen las observaciones?']"
            @close="modalIA = false" />
    ...
    <div class="ht-card">
      ...
      <div class="ht-split">
        ...
        <div class="ht-gen">
          <div class="ht-f"><span class="ht-lbl">Parte diario de fecha</span><input v-model="fecha" type="date" class="ht-inp" /></div>
          <label class="ht-chk"><input type="checkbox" v-model="noRegistrados" /> Mostrar solo no registrados</label>
          <button class="ht-btn ok blk" :disabled="!seleccionados || cargando" @click="generar">{{ cargando ? '⟳…' : 'Parte Diario General' }}</button>
          <button class="ht-btn ok" v-if="filas.length" @click="imprimir">🖨 Imprimir</button>
        </div>
      </div>

      <div v-if="filas.length" class="ht-grid-wrap">
        <table class="ht-tabla">
          <thead><tr><th>Legajo</th><th>Nombre</th><th>Entrada</th><th>2ª Entrada</th><th>Observación</th></tr></thead>
          <tbody><tr v-for="f in filas" :key="f.cod" :class="{ obs: f.observa }">
            <td>{{ f.legajo }}</td><td>{{ f.nombre }}</td><td class="c">{{ hm(f.entra1) }}</td><td class="c">{{ hm(f.entra2) }}</td><td>{{ f.observa }}</td>
          </tr></tbody>
        </table>
      </div>
...
import ParteDiarioAyuda from '@/components/ParteDiarioAyuda.vue'
...
const cargandoEmp = ref(false); const cargando = ref(false); const msg = ref(''); const noRegistrados = ref(false)
const filas = ref<any[]>([])
...
const generar = async () => {
  ...
  try { const { data } = await api.post('/reloj/parte-diario', { codigos, fecha: fecha.value, noRegistrados: noRegistrados.value }); filas.value = data.parte ?? []; if (!filas.value.length) msg.value = 'Sin registros para esa fecha.' }
  catch (e: any) { msg.value = e?.response?.data?.message ?? 'No se pudo generar el parte.' }
  finally { cargando.value = false }
}

const imprimir = () => {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  doc.setFont('helvetica', 'bold'); doc.setFontSize(13); doc.text(`Parte Diario de Personal — ${fmt(fecha.value)}`, 14, 16)
  const cols = ['Legajo', 'Nombre', 'Entrada', '2ª Ent.', 'Observación']; const xs = [14, 34, 96, 116, 138]; let y = 26
  doc.setFontSize(9); doc.setFont('helvetica', 'bold'); cols.forEach((c, i) => doc.text(c, xs[i], y)); doc.setFont('helvetica', 'normal'); y += 6
  for (const f of filas.value) { if (y > 285) { doc.addPage(); y = 20 } [String(f.legajo), f.nombre.slice(0, 34), hm(f.entra1), hm(f.entra2), f.observa.slice(0, 40)].forEach((c, i) => doc.text(String(c), xs[i], y)); y += 5 }
  cerrarPdf(); pdfNombre.value = 'PARTE_DIARIO.pdf'; pdfUrl.value = URL.createObjectURL(doc.output('blob'))
}
...
.ht-tabla td { padding:5px 8px; border-bottom:1px solid #eef2f7; white-space:nowrap; color:#1e293b; } .ht-tabla td.c { text-align:center; }
.ht-tabla tbody tr.obs td { background:#e0f2fe; }
