<!-- TallesView.vue — ABM de Talles (tabla talles). Sin baja (igual que FoxPro). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">📏</div>
      <div class="cs-tx"><h1>Talles</h1><p>{{ lista.length }} talle{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/talles" titulo="Asistente IA — Talles"
            subtitulo="Preguntá sobre el ABM de talles"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego un talle?','¿Dónde se usan?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin talles cargados</div>
      <table v-else class="cs-tabla">
        <thead><tr><th style="width:90px" class="ord" @click="ordenar('cod')">Código{{ flecha('cod') }}</th><th class="ord" @click="ordenar('nombre')">Descripción{{ flecha('nombre') }}</th><th style="width:90px;text-align:center">Editar</th></tr></thead>
        <tbody>
          <tr v-for="e in listaFiltrada" :key="e.cod">
            <td class="cs-cod">{{ e.cod }}</td><td>{{ e.nombre }}</td>
            <td style="text-align:center"><button class="cs-i" @click="abrirEditar(e)">✏️</button></td>
          </tr>
        </tbody>
      </table>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar talle' : 'Nuevo talle' }}</h3>
          <label>Descripción</label>
          <input ref="inputNom" v-model="form.nombre" maxlength="30" @keyup.enter="guardar" />
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Talles</h3>
          <ul class="cs-help">
            <li>Administra los <b>talles</b> (XS, S, M, L, XL…) que se usan en la gestión de uniformes y EPP.</li>
            <li>El <b>código</b> lo asigna el sistema; el <b>nombre</b> es obligatorio.</li>
            <li>La baja está deshabilitada para no afectar entregas existentes.</li>
          </ul>
...
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/talles')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) { await api.put(`/talles/${form.value.cod}`, { nombre: form.value.nombre }); flash('Talle actualizado') }
    else { await api.post('/talles', { nombre: form.value.nombre }); flash('Talle creado') }
...
.cs-i { background:none; border:none; cursor:pointer; font-size:15px; padding:2px 5px; }
