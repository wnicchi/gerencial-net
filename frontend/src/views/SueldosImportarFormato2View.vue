<!--
  SueldosImportarFormato2View.vue — Importar del Estudio - Formato 2 (sueldos_importar_formato_estudio_2).
  Igual que Importar del Estudio, pero con UNA sola columna IMPORTE: positivo = haber, negativo = deducción.
  Reutiliza el endpoint POST /sueldos-importar (el signo se resuelve acá).
-->
<template>
  <div class="ia-view">
    <div class="ia-cab">
      <div class="ia-cab-ico">📥</div>
      <div class="ia-cab-tx"><h1>Importar del Estudio - Formato 2</h1><p>Una sola columna IMPORTE: positivo = haber, negativo = deducción</p></div>
    ...
    <SueldosImportarFormato2Ayuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/sueldos-importar" titulo="Asistente IA — Importar del Estudio (Formato 2)"
            subtitulo="Preguntá cómo importar desde el Excel"
            :sugerencias="['¿Cómo se interpreta el IMPORTE?','¿Qué pasa si un CUIL no existe?','¿Reemplaza la liquidación anterior?']"
            @close="modalIA = false" />

    <div class="ia-card">
      <div class="ia-nota">📌 De la columna <b>IMPORTE</b>: los valores <b>negativos</b> se cargan como <b>DEDUCCIONES</b> y los <b>positivos</b> como <b>HABERES</b>.</div>
...
import SueldosImportarFormato2Ayuda from '@/components/SueldosImportarFormato2Ayuda.vue'
...
const map = reactive<{ cuil: number; codigo: number; detalle: number; cantidad: number; importe: number }>({ cuil: -1, codigo: -1, detalle: -1, cantidad: -1, importe: -1 })
const mapeos = [
  { k: 'cuil', lbl: 'CUIL' }, { k: 'codigo', lbl: 'Código' }, { k: 'detalle', lbl: 'Detalle' },
  { k: 'cantidad', lbl: 'Cantidad' }, { k: 'importe', lbl: 'Importe (+ haber / − deducción)' },
]
const claves = ['cuil', 'codigo', 'detalle', 'cantidad', 'importe'] as const
...
// Parseo AR ("1.234,56" / "-1.234,56") a número con signo.
const num = (v: string) => {
...
const proceder = async () => {
  ...
  if (!claves.every(k => (map as any)[k] >= 0)) return flash('Indicá las columnas de CUIL, Código, Detalle, Cantidad e Importe.', true)
  ...
  try {
    const rows = filas.filter(f => f.ok).map(f => {
      const imp = num(cel(f, map.importe))
      return {
        cuil: cel(f, map.cuil), codigo: cel(f, map.codigo), detalle: cel(f, map.detalle), cantidad: cel(f, map.cantidad),
        haber: imp >= 0 ? imp : 0, deduce: imp < 0 ? -imp : 0,
      }
    })
...
.ia-nota { background:#fffbeb; border:1px solid #fde68a; color:#92400e; padding:10px 14px; border-radius:8px; font-size:13px; margin-bottom:14px; }
...
.ia-grid-wrap { overflow:auto; border:1px solid #e2e8f0; border-radius:8px; max-height:44vh; }
