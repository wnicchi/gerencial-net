<!-- BancoVariosConsultarView.vue — Bcos. Varios (Santander) - Consultar (empleados_consultar_bco_varios). -->
<template>
  <div class="bc-view">
    <div class="bc-cab">
      <div class="bc-cab-ico">🔍</div>
      <div class="bc-cab-tx"><h1>Bcos. Varios (Santander) - Consultar</h1><p>Lotes de pago a otros bancos — autorización y regeneración</p></div>
      <BancoSantanderLogo :alto="34" class="bc-logo" />
    ...
    <BancoVariosConsultarAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-varios-consultar" titulo="Asistente IA — Bcos. Varios Consultar"
            subtitulo="Preguntá sobre los lotes y autorizaciones"
            :sugerencias="['¿Qué hace Generar el movimiento de pagos?','¿Qué es un lote anulado?','¿Cómo regenero un TXT?']"
            @close="modalIA = false" />
    ...
    <div class="bc-filtros" v-enter-next>
      ...
      <span v-if="lotes.length" class="bc-info">{{ lotes.length }} lotes · seleccionado: {{ lotesSel.length ? lotesSel[0].nro : '—' }}</span>
    </div>

    <div class="bc-cols">
      <div class="bc-panel">
        <div class="bc-panel-tit">Lotes (Haberes Bcos. Varios)</div>
        <div class="bc-wrap">
          <table class="bc-grid">
    ...
    <div class="bc-acciones">
      ...
      <div class="bc-derecha">
        ...
        <div class="bc-gen">
          ...
          <div class="bc-f"><span class="bc-lbl">Orden del día</span><input v-model.number="gen.orden" type="number" class="bc-orden" /></div>
          <label class="bc-chk"><input type="checkbox" v-model="gen.enviaAlBanco" /> Enviar al banco</label>
          <div class="bc-gen-btns">
            <button class="bc-btn-txt" :disabled="generando || !loteActivo" @click="generarNuevo('txt')">{{ generando ? '⟳…' : 'GENERAR NUEVO TXT' }}</button>
            <button class="bc-btn-txt excel" :disabled="generando || !loteActivo" @click="generarNuevo('excel')">GENERAR NUEVO EXCEL</button>
          </div>
        </div>
      </div>
    </div>
...
import BancoVariosConsultarAyuda from '@/components/BancoVariosConsultarAyuda.vue'
...
const gen = reactive({ fechaEnvio: _iso(hoy), fechaImputacion: _iso(hoy), orden: 1, enviaAlBanco: false })
...
const cargarLotes = async () => {
  cargando.value = true
  try { lotes.value = (await api.get('/bco-varios/lotes', { params: { empresa: empresa.value } })).data.lotes || []; bene.value = []; loteActivo.value = 0 }
...
const abrirLote = async (l: Lote) => {
  ...
  try { bene.value = (await api.get('/bco-varios/beneficiarios', { params: { lote: l.nro } })).data.beneficiarios || [] }
...
const generarMovimiento = async () => {
  ...
  if (!sel.length) return flash('Seleccioná el lote a confirmar el pago.', true)
  ...
  if (!confirm(`¿Generar el pago en el sistema para el lote ${sel[0].nro}? Quedará para que autorice Gerencia.`)) return
  procesando.value = true
  try {
    const { data } = await api.post('/bco-varios/generar-movimiento', {
...
const eliminarLotes = async () => {
  const sel = lotesSel.value
  if (!sel.length) return flash('Seleccioná el lote a eliminar.', true)
  if (!confirm(`¿Anular el lote ${sel[0].nro} de varhaberes?`)) return
  try {
    await api.post('/bco-varios/eliminar', { lotes: sel.map(l => l.nro), nroBancario: movimiento.nroBancario || 0 })
    sel.forEach(l => { l.anulado = true; l.elegir = false })
    flash('Lote anulado.')
  } catch (e: any) { flash(e?.response?.data?.message ?? 'No se pudo anular.', true) }
}

const generarNuevo = async (tipo: 'txt' | 'excel') => {
  ...
  if (!confirm(`Se creará un NUEVO lote y su ${tipo.toUpperCase()} con los empleados/montos actuales. ¿Continuar?`)) return
  generando.value = true
  try {
    const { data } = await api.post('/bco-varios/generar-nuevo', {
      tipo, loteOrigen: loteActivo.value, fechaEnvio: gen.fechaEnvio, fechaImputacion: gen.fechaImputacion,
      orden: gen.orden, enviaAlBanco: gen.enviaAlBanco, beneficiarios: bene.value.map(b => ({ emp: b.emp, mon: b.mon })),
    })
...
const imprimirPdf = async () => {
  ...
  const url = pdfAcreditaciones({
    ...
    filas: bene.value.map(b => ({ legajo: b.leg, nombre: b.emd, cuenta: `${b.cta}/${String(b.suc).padStart(2, '0')}`, importe: b.mon })),
  })
...
.bc-f { display:flex; flex-direction:column; gap:4px; } .bc-lbl { font-size:12px; font-weight:700; color:#b30000; }
.bc-sel { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; min-width:200px; color:#1e293b; }
.bc-btn-ok { background:#b30000; color:#fff; border:none; padding:8px 22px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-ok:disabled { background:#cbd5e1; }
...
.bc-panel-tit { background:#b30000; color:#fff; font-size:12px; font-weight:700; padding:7px 12px; }
...
.bc-grid tbody tr.activo td { outline:2px solid #ec0000; outline-offset:-2px; }
...
.bc-tfoot td { background:#7f1d1d !important; color:#fff !important; font-weight:800; padding:7px 8px; }
...
.bc-btn-mov { background:#b30000; color:#fff; border:none; padding:12px; border-radius:8px; cursor:pointer; font-size:13px; font-weight:800; } .bc-btn-mov:disabled { background:#cbd5e1; }
...
.bc-orden { border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; width:70px; background:#fef9c3; }
.bc-chk { display:flex; align-items:center; gap:6px; font-size:13px; color:#1e293b; }
.bc-gen-btns { margin-left:auto; display:flex; flex-direction:column; gap:8px; align-items:stretch; }
.bc-btn-txt { background:#b30000; color:#fff; border:none; padding:10px 16px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bc-btn-txt.excel { background:#16a34a; } .bc-btn-txt:disabled { background:#cbd5e1; }
