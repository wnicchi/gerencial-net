<!-- RubrosRopaView.vue — ABM de Rubros de Ropa/EPP (tabla roparubro). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">🏷️</div>
      <div class="cs-tx"><h1>Rubros de Ropa/EPP</h1><p>{{ lista.length }} rubro{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/rubros-ropa" titulo="Asistente IA — Rubros de Ropa/EPP"
            subtitulo="Preguntá sobre el ABM de rubros"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego un rubro?','¿Por qué no me deja eliminar uno?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin rubros cargados</div>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar rubro' : 'Nuevo rubro' }}</h3>
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Rubros de Ropa/EPP</h3>
          <ul class="cs-help">
            <li>Administra los <b>rubros</b> que clasifican las prendas y EPP (ej. Vestimenta reglamentaria, Elementos de seguridad).</li>
            <li>El <b>código</b> lo asigna el sistema; el <b>nombre</b> es obligatorio.</li>
            <li>No se puede eliminar un rubro usado por alguna prenda/EPP.</li>
          </ul>
...
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/rubros-ropa')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) { await api.put(`/rubros-ropa/${form.value.cod}`, { nombre: form.value.nombre }); flash('Rubro actualizado') }
    else { await api.post('/rubros-ropa', { nombre: form.value.nombre }); flash('Rubro creado') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar el rubro "${e.nombre}"?`)) return
  try { await api.delete(`/rubros-ropa/${e.cod}`); flash('Rubro eliminado'); await cargar() }
