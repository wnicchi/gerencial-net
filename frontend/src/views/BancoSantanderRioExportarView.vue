<!-- BancoSantanderRioExportarView.vue — Bco. Santander Río - Exportar (empleados_exportar_bco_santander_rio). -->
<template>
  <div class="bs-view">
    <div class="bs-cab">
      <div class="bs-cab-ico">📤</div>
      <div class="bs-cab-tx"><h1>Bco. Santander Río - Exportar</h1><p>Archivo de acreditación de sueldos para el Banco Santander Río (por CBU)</p></div>
      <BancoSantanderLogo :alto="38" class="bs-logo" />
    ...
    <BancoSantanderRioAyuda v-if="modalAyuda" @close="modalAyuda = false" />
    <ChatIA v-if="modalIA" endpoint="/ia/bco-santander-rio" titulo="Asistente IA — Bco. Santander Río Exportar"
            subtitulo="Preguntá sobre la exportación al banco"
            :sugerencias="['¿Por qué un empleado figura en cero?','¿Qué significan las filas rojas?','¿TXT o Excel?']"
            @close="modalIA = false" />
...
import BancoSantanderRioAyuda from '@/components/BancoSantanderRioAyuda.vue'
...
const f = reactive({ empresa: 0, contratista: 0, lugar: 0, convenio: 0, categoria: 0, empleado: 0, mes: _prev.getMonth() + 1, anio: _prev.getFullYear(), tipoRemu: 0, tipoSueldo: 0, banco: 1, anticipos: false })
...
const consultar = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/bco-santander-rio/consultar', { params })
...
const generar = async (tipo: 'txt' | 'excel') => {
  ...
  try {
    ...
    const { data } = await api.post('/bco-santander-rio/generar', payload)
...
onMounted(async () => {
  ...
  const san = cat.bancos.find((b: any) => b.nombre.toUpperCase().includes('SANTANDER'))
  if (san) f.banco = san.cod
})
</script>
...
.bs-total-box { margin-left:auto; display:flex; align-items:center; gap:12px; background:linear-gradient(120deg,#b30000,#ec0000); color:#fff; padding:9px 16px; border-radius:9px; box-shadow:0 6px 18px rgba(179,0,0,.35); }
...
.bs-btn-gen { background:#2563eb; color:#fff; border:none; padding:10px 18px; border-radius:6px; cursor:pointer; font-size:13px; font-weight:700; } .bs-btn-gen.excel { background:#16a34a; } .bs-btn-gen:disabled { background:#cbd5e1; }
