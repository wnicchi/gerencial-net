<!-- AreasTematicasView.vue — ABM de Áreas Temáticas (tabla tema). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">📚</div>
      <div class="cs-tx"><h1>Áreas Temáticas</h1><p>{{ lista.length }} área{{ lista.length === 1 ? '' : 's' }}</p></div>
      <button class="cs-ia" @click="modalIA = true">🤖 IA</button>
      <button class="cs-ayuda" @click="ayuda = true">❓ Ayuda</button>
      <button class="cs-nuevo" @click="abrirNuevo">＋ Nueva</button>
    </div>

    <ChatIA v-if="modalIA" endpoint="/ia/areas-tematicas" titulo="Asistente IA — Áreas Temáticas"
            subtitulo="Preguntá sobre el ABM de áreas temáticas"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego un área?','¿Dónde se usan?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin áreas temáticas cargadas</div>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar área temática' : 'Nueva área temática' }}</h3>
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Áreas Temáticas</h3>
          <ul class="cs-help">
            <li>Administra las <b>áreas temáticas</b> de las capacitaciones (ej. Seguridad e Higiene, Medicina Laboral).</li>
            <li>El <b>código</b> lo asigna el sistema; la <b>descripción</b> es obligatoria.</li>
            <li>Estas áreas se eligen al cargar una jornada de capacitación.</li>
          </ul>
...
const listaFiltrada = computed(() => {
  ...
  return base.slice().sort((a, b) => k === 'cod' ? (a.cod - b.cod) * dir : String(a.nombre).localeCompare(String(b.nombre), 'es', { numeric: true }) * dir)
})
const flash = (t: string, e = false) => { msg.value = t; msgError.value = e; setTimeout(() => msg.value = '', 3000) }
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/areas-tematicas')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  if (!form.value.nombre.trim()) { modalError.value = 'La descripción es obligatoria.'; return }
  guardando.value = true; modalError.value = ''
  try {
    if (editando.value) { await api.put(`/areas-tematicas/${form.value.cod}`, { nombre: form.value.nombre }); flash('Área actualizada') }
    else { await api.post('/areas-tematicas', { nombre: form.value.nombre }); flash('Área creada') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar el área "${e.nombre}"?`)) return
  try { await api.delete(`/areas-tematicas/${e.cod}`); flash('Área eliminada'); await cargar() }
...
.cs-md input { width:100%; border:1px solid #c8d8ea; border-radius:7px; padding:9px 11px; font-size:14px; margin-top:5px; outline:none; box-sizing:border-box; color:#1e293b; }
