<!-- FrecuenciasView.vue — ABM de Frecuencias (tabla frecuencia). -->
<template>
  <div class="cs-view">
    <div class="cs-cab">
      <div class="cs-ico">🔄</div>
      <div class="cs-tx"><h1>Frecuencias</h1><p>{{ lista.length }} frecuencia{{ lista.length === 1 ? '' : 's' }}</p></div>
    ...
    <ChatIA v-if="modalIA" endpoint="/ia/frecuencias" titulo="Asistente IA — Frecuencias"
            subtitulo="Preguntá sobre el ABM de frecuencias"
            :sugerencias="['¿Para qué sirve este módulo?','¿Cómo agrego una frecuencia?','¿Dónde se usan?','¿El código se asigna solo?']"
            @close="modalIA = false" />

    <div class="cs-card">
      <input v-model="filtro" class="cs-search" placeholder="🔍 Buscar por nombre…" />
      <div v-if="cargando" class="cs-vacio">⟳ Cargando…</div>
      <div v-else-if="!listaFiltrada.length" class="cs-vacio">Sin frecuencias cargadas</div>
      <table v-else class="cs-tabla">
        <thead><tr><th style="width:90px">Código</th><th>Descripción</th><th style="width:110px;text-align:center">Acciones</th></tr></thead>
        <tbody>
    ...
    <Teleport to="body">
      <div v-if="modal" class="cs-ov" @click.self="modal = false">
        <div class="cs-md" v-enter-next>
          <h3>{{ editando ? 'Editar frecuencia' : 'Nueva frecuencia' }}</h3>
      ...
      <div v-if="ayuda" class="cs-ov" @click.self="ayuda = false">
        <div class="cs-md">
          <h3>❓ Ayuda — Frecuencias</h3>
          <ul class="cs-help">
            <li>Administra las <b>frecuencias de uso</b> (ej. Alta, Media, Baja) que se usan al describir las tareas de cada puesto.</li>
            <li>El <b>código</b> lo asigna el sistema; el <b>nombre</b> es obligatorio.</li>
            <li>No se puede eliminar una frecuencia usada por alguna tarea.</li>
          </ul>
...
const listaFiltrada = computed(() => { const q = filtro.value.trim().toLowerCase(); return q ? lista.value.filter(e => e.nombre.toLowerCase().includes(q)) : lista.value })
const flash = (t: string, e = false) => { msg.value = t; msgError.value = e; setTimeout(() => msg.value = '', 3000) }
const cargar = async () => { cargando.value = true; try { lista.value = (await api.get('/frecuencias')).data } catch (e) { console.error(e) } finally { cargando.value = false } }
...
const guardar = async () => {
  ...
  try {
    if (editando.value) { await api.put(`/frecuencias/${form.value.cod}`, { nombre: form.value.nombre }); flash('Frecuencia actualizada') }
    else { await api.post('/frecuencias', { nombre: form.value.nombre }); flash('Frecuencia creada') }
...
const eliminar = async (e: Item) => {
  if (!confirm(`¿Eliminar la frecuencia "${e.nombre}"?`)) return
  try { await api.delete(`/frecuencias/${e.cod}`); flash('Frecuencia eliminada'); await cargar() }
