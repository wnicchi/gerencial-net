...
/**
 * BancoFrancesConsultarController — Bco. Francés (BBVA) - Consultar (empleados_consultar_bco_frances.scx).
 *
 * Gemelo de los otros "Consultar" pero sobre FRHABERES/FRHAB_ITEM y banco de Gestión 3 (BBVA):
 *  - lotes() / beneficiarios().
 *  - generarMovimiento(): registra el lote en gestion PAGOS_AUTORIZAR (OPA_BAN=3, "BBVA FRANCES") y marca FRH_PAG.
 *  - eliminar(): FRH_ANU='S' (+ OPA_AUTANU='S' en Gestión si estaba pagado).
 *  - generarNuevo(): regenera el TXT BBVA (250 chars) con los beneficiarios editados, releyendo el padrón,
 *    creando un NUEVO lote. El ordenante del archivo es el nombre del lote de origen; el concepto, su FRH_COD.
 */
class BancoFrancesConsultarController extends Controller
{
    private const BANCO_GESTION = 3; // BBVA

    /** @route GET /api/bco-frances/lotes?empresa= */
    public function lotes(Request $request): JsonResponse
    {
        $q = DB::table('FRHABERES');
    ...
    /** @route GET /api/bco-frances/beneficiarios?lote= */
    public function beneficiarios(Request $request): JsonResponse
    {
        $lote = (int) $request->validate(['lote' => 'required|integer'])['lote'];
        $rows = DB::table('FRHAB_ITEM as i')->leftJoin('personal as p', 'i.FRI_EMP', '=', 'p.PER_COD')
    ...
    /** @route POST /api/bco-frances/generar-movimiento */
    public function generarMovimiento(Request $request): JsonResponse
    {
        ...
        foreach ($d['lotes'] as $nro) {
            $nro = (int) $nro;
            $frh = DB::table('FRHABERES')->where('FRH_NRO', $nro)->first();
            ...
            try {
                ...
                if ($previo) {
                    DB::table('FRHABERES')->where('FRH_NRO', $nro)->update(['FRH_PAG' => 1]);
                ...
                $gx->table('PAGOS_AUTORIZAR')->insert([
                    ...
                    'OPA_BAD' => 'BBVA FRANCES', 'OPA_DET' => $tipoDesc, 'OPA_IMP' => round((float) $frh->FRH_MON, 2),
                ...
                \Log::error('[BcoFrances generarMovimiento] lote ' . $nro . ': ' . $e->getMessage());
    ...
    /** @route POST /api/bco-frances/eliminar */
    public function eliminar(Request $request): JsonResponse
    {
        ...
        foreach ($d['lotes'] as $nro) {
            ...
            $frh = DB::table('FRHABERES')->where('FRH_NRO', $nro)->first();
            if (!$frh) continue;
            DB::table('FRHABERES')->where('FRH_NRO', $nro)->update(['FRH_ANU' => 'S']);
            $anulados++;
            if ((int) $frh->FRH_PAG === 1 && $nroBancario > 0) {
                ...
                } catch (\Throwable $e) { \Log::error('[BcoFrances eliminar] ' . $e->getMessage()); }
            }
        }
        return response()->json(['message' => "Lote(s) anulado(s): $anulados.", 'anulados' => $anulados]);
    }

    /** @route POST /api/bco-frances/generar-nuevo */
    public function generarNuevo(Request $request): JsonResponse
    {
        ...
        $origen = DB::table('FRHABERES')->where('FRH_NRO', (int) $d['loteOrigen'])->first();
        ...
        DB::transaction(function () use ($filas, $origen, $codEmpresa, $codConvenio, $concepto, $fEnvio, $fImput, $usuario, $total, &$nuevoNro) {
            $nuevoNro = (int) max((int) DB::table('FRHABERES')->max('FRH_NRO'), (int) DB::table('FRHAB_ITEM')->max('FRI_NRO')) + 1;
        ...
        return response()->json([
            ...
            'contenido' => $contenido, 'mime' => 'text/plain', 'lote' => $nuevoNro, 'total' => $total, 'lineas' => count($filas) * 4 + 2,
        ]);
    }
